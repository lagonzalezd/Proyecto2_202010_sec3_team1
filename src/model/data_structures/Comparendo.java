...
public class Comparendo implements Comparable<Comparendo> {
    ...
    public int compareTo(Comparendo ob) {
        ...
        } else if (indicadorComparacion.equalsIgnoreCase("INFRACCION")) {

            if (TIPO_SERVICIO.equals(ob.getTIPO_SERVICIO())) {
                return INFRACCION.compareTo(ob.getINFRACCION()) == 0 ? 0 : INFRACCION.compareTo(ob.getINFRACCION()) > 0 ? 1 : -1;

            } else {
                if ((INFRACCION.equals("Pùblico") || INFRACCION.equals("Oficial")) && ob.getINFRACCION().equals("Particular"))
                    return 1;

                else {
                    if ((ob.getINFRACCION().equals("Público") || ob.getINFRACCION().equals("Oficial")) && INFRACCION.equals("Particular"))
                        return -1;

                    else {
                        if (INFRACCION.equals("Público") && ob.getINFRACCION().equals("Oficial"))
                            return 1;

                        else {
                            if (ob.getINFRACCION().equals("Público") && INFRACCION.equals("Oficial"))
                                return -1;
